...
void ShooterSubsystem::TurnOn(){
	...
	//.7 ON PERCENT MODE IS 2940 RPMs
	//.5 ON PERCENT MODE IS 2100 RPMs
	//.2 ON PERCENT MODE IS 760 RPMs
	...
	//talonController->Set(.70);
	//talonController2->Set(.70);
	...
	double p = .1;			/*Kp 0.3  ?controls speed?*/
	double i = 0;		/*Ki 0.03  ???? */
	double d = 10;			/*Kd 3  ?longer to rev up?*/
	double f = .0336;		/*Kf 0.0003 */
	double f2 = .0342;
	int izone = 0;		/* encoder ticks/ analog units 300 */
	double ramprate = 2;	/* volts per second, =>0% to 100% in 250ms, 48 */
	int profile = 0;
	...
	//4340rpm @ set(6) p=1 ,i=.3, d=3, f=3
	talonController->Set(2000);
